...
S3Backend.prototype.readdir = function(offset, callback) {
  ...
  this.client.list({ prefix: dest, delimiter: '/' }, function(err, data) {
    ...
    // files
    var out = _.map(data.Contents, function(item) {
      return item.Key.slice(prefix.length);
    });
    // directories
    out = out.concat(_.map(data.CommonPrefixes, function(item) {
...
var DomainObject = {
  ...
  , fetch: function(callback) {
    ...
    self.backend.read(dest, function(error, data) {
      self._data = data;
      callback(error, self);
    });
  }
...
var Viz = DomainObject.extend({
    __type__: 'viz'
  // Return list of Viz objects (toJSON'd) owned by this user
  , getByOwner: function(ownerId, callback) {
    var backend = getBackend();
    backend.readdir(ownerId, function(err, data) {
      var out = [];
      var data = _.filter(data, function(fileOrDirName) {
        return (fileOrDirName !== 'data.json');
      });
      function extract(vizName, cb) {
        var viz = Viz.create({owner: ownerId, name: vizName});
